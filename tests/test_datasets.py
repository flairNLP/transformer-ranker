...
def test_datacleaner(task_type, dataset_name, downsampling_ratio):
    ...
    assert task_category == task_type, (
        f"Task type mismatch: expected '{task_type}', got '{task_category}'" f"in dataset '{dataset_name}'"
    )

    assert isinstance(texts, list) and len(texts) > 0, ("Sentences/tokens list is empty in dataset %s", dataset_name)
    ...
    if task_type == "text classification":
        ...
        for sentence in texts:
            ...
            for token in sentence:
                assert isinstance(
                    token, str
                ), f"Non-str token ({type(token)}) found within a sentence in dataset {dataset_name}."
